...
import std/[strutils, os, tables, options, parseutils]
...
# ==============================================================================
# IP Parsing & Private / Bogon Detection Helpers
# ==============================================================================
...
proc parseIpv4Octets*(ip: string, octets: var array[4, int]): bool =
  ...
  for i in 0..3:
    var val = 0
    let parsed = parseutils.parseInt(ip, val, idx)
    if parsed <= 0 or val < 0 or val > 255:
      return false
    octets[i] = val
    idx += parsed
    if i < 3:
      if idx >= ip.len or ip[idx] != '.':
        return false
      inc idx
  return idx == ip.len
...
proc isPrivateIp*(ip: string): bool =
  ...
  ## Carrier-Grade NAT (CGNAT), or IPv6 private/local subnets.
  if ip.len == 0:
    return false

  let lower = ip.toLowerAscii()
  if lower == "127.0.0.1" or lower == "localhost" or lower == "::1" or lower == "::":
    return true

  var ipNum: uint32
  if parseIpv4ToUint32(ip, ipNum):
    # 127.0.0.0/8 Loopback (2130706432 .. 2147483647)
    if (ipNum and 0xFF000000'u32) == 0x7F000000'u32:
      return true
    # 10.0.0.0/8 RFC 1918 (167772160 .. 184549375)
    if (ipNum and 0xFF000000'u32) == 0x0A000000'u32:
      return true
    # 172.16.0.0/12 RFC 1918 (2886729728 .. 2887778303)
    if (ipNum and 0xFFF00000'u32) == 0xAC100000'u32:
      return true
    # 192.168.0.0/16 RFC 1918 (3232235520 .. 3232301055)
    if (ipNum and 0xFFFF0000'u32) == 0xC0A80000'u32:
      return true
    # 169.254.0.0/16 Link-Local (2851995648 .. 2852061183)
    if (ipNum and 0xFFFF0000'u32) == 0xA9FE0000'u32:
      return true
    # 100.64.0.0/10 Carrier-Grade NAT RFC 6598 (1681915904 .. 1686110207)
    if (ipNum and 0xFFC00000'u32) == 0x64400000'u32:
      return true
    # 0.0.0.0/8 Current Network
    if (ipNum and 0xFF000000'u32) == 0x00000000'u32:
      return true
    # 224.0.0.0/4 Multicast
    if (ipNum and 0xF0000000'u32) == 0xE0000000'u32:
      return true
    # 240.0.0.0/4 Reserved
    if (ipNum and 0xF0000000'u32) == 0xF0000000'u32:
      return true
    # 255.255.255.255 Broadcast
    if ipNum == 0xFFFFFFFF'u32:
      return true
    return false
  ...
  if parseIpv6ToBytes(ip, v6Bytes):
    ...
    var isLoopback = true
    for i in 0..14:
      if v6Bytes[i] != 0: isLoopback = false
    if isLoopback and v6Bytes[15] == 1:
      return true

    # fe80::/10 Link-local (0xFE80 to 0xFEBF)
    if v6Bytes[0] == 0xFE'u8 and (v6Bytes[1] and 0xC0'u8) == 0x80'u8:
      return true

    # fc00::/7 Unique Local Address (ULA) (0xFC00 to 0xFDFF)
    if (v6Bytes[0] and 0xFE'u8) == 0xFC'u8:
      return true
